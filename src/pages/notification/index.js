...
const Notification = () => {
  ...
  const sortFunc = (params) => {
    ...
    if (params === "asc") {
      let sortedData = defaultData.sort((a, b) => a.title.localeCompare(b.name));
      setData([...sortedData]);
    } else if (params === "dsc") {
      let sortedData = defaultData.sort((a, b) => b.title.localeCompare(a.name));
      setData([...sortedData]);
    }
  };
  ...
  const onEditSubmit = async(submitData) => {
    const {  edit_title, edit_description} = submitData;
    const {edit_user} = formData;
    ...
    try{
    const res = await fetch(BASE_URL + `notification/${editId}`,{
        method : "put",
        headers : {
            "Content-Type" : "application/json",
            'Authorization': 'Bearer ' + accessToken,
  ...
  const onEditClick = (id) => {
    data.forEach((item) => {
      if (item._id === id) {
        let use = users.filter(user => user._id === item.user._id);
        setFormData({
          edit_user : use.length > 0 ? {label : use[0].firstName + ' ' + use[0].lastName, value : use[0]._id} : item.user._id,
  ...
  const onActionClick = async(e) => {
    if (actionText === "delete") {
      ...
      try{
        const res = await fetch(BASE_URL + `notification/bulk-delete`,{
            method : "post",
            headers : {
                "Content-Type" : "application/json",
                'Authorization': 'Bearer ' + accessToken,
  ...
  const exportTocsv = () => {
    var csv = '_id,User Name,Title,Description,Created At\n';
    data.forEach(function(row){
      csv += (row._id + ',' + row.user.firstName + ' ' + row.user.lastName + ',' + row.title + ',' + row.description +  ',' + row.createdAt + '\n')
    })
    ...
    hiddenElement.download = 'User Notifications.csv';  
    hiddenElement.click();  
  }
  ...
  return (
    <React.Fragment>
      <Head title="User Notifications"></Head>
      <Content>
        <BlockHead size="sm">
          <BlockBetween>
            <BlockHeadContent>
              ...
              <BlockDes className="text-soft">
                <p>You have total {data.length} notifications.</p>
              </BlockDes>
            </BlockHeadContent>
            <BlockHeadContent>
            ...
            <div className="p-2">
              ...
              <div className="mt-4">
                ...
                <Col md="6">
                    <FormGroup>
                      <label className="form-label">Loan Type</label>
                      <RSelect options={filterLoanType} name="edit_loan_type" placeholder="Enter User" defaultValue={formData.edit_user} onChange={(e) => {
                    ...
                    <Col md="6">
                      ...
                      <input
                        ...
                        type="text"
                        name="edit_title"
                        defaultValue={formData.edit_title}
                        placeholder="Enter Title"
                        ref={register({ required: "Title is required" })}
                      />
                      {errors.edit_title && <span className="invalid">{errors.edit_title.message}</span>}
                      </FormGroup>
                    </Col>
                    <Col md="6">
                      <FormGroup>
                      ...
                      <input
                        ...
                        defaultValue={formData.edit_description}
                        placeholder="Enter Description"
                        ref={register({ required: "Description is required" })}
                      />
                      {errors.edit_description && <span className="invalid">{errors.edit_description.message}</span>}
                      </FormGroup>
                    </Col>

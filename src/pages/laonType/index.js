...
const LoanType = () => {
  ...
  const bulkActionOptions = [
    { value: "delete", label: "Delete Loan" },
  ];
  ...
  const onEditSubmit = async(submitData) => {
    const { name,edit_name } = submitData;
    let submittedData;
    submittedData = {
        name : name,
        edit_name : edit_name
    ...
    try{
    const res = await fetch(BASE_URL + `loan-type/${editId}`,{
        method : "put",
        headers : {
            "Content-Type" : "application/json",
            'Authorization': 'Bearer ' + accessToken,
  ...
  const onActionClick = async(e) => {
    if (actionText === "delete") {
      ...
      try{
        const res = await fetch(BASE_URL + `loan-type/bulk-delete`,{
            method : "post",
            headers : {
                "Content-Type" : "application/json",
                'Authorization': 'Bearer ' + accessToken,
  ...
  return (
    <React.Fragment>
      <Head title="Loan Typ Lists"></Head>
      <Content>
        <BlockHead size="sm">
          <BlockBetween>
            <BlockHeadContent>
        ...
        <Modal isOpen={modal.edit} toggle={() => setModal({ edit: false })} className="modal-dialog-centered" size="lg">
          <ModalBody>
            ...
            <div className="p-2">
              <h5 className="title">Update Loan Type</h5>
              <div className="mt-4">

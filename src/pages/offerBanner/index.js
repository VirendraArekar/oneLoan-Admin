...
const OfferBanner = () => {
  ...
  const loadBannerData = async() => {
    setLoading(true);
    const res = await fetch(BASE_URL + 'offer-banner',{
      method : "get",
      headers : {
  ...
  const searchData = (keyword) => {
    ...
    result = data.filter((item) => {
         return item.loan.name.search(value) != -1;
    })
    setData([...result])
  }
  ...
  const onEditSubmit = async(submitData) => {
    ...
    var data = new FormData();
    data.append('loan', typeof edit_loanType === 'object' ? edit_loanType.value : edit_loanType);
    data.append('image',submitEditImage);

    try{
    const res = await fetch(BASE_URL + `offer-banner/${editId}`,{
        method : "put",
        headers : {
            // "Content-Type" : "application/json",
            'Authorization': 'Bearer ' + accessToken,
  ...
  const onEditClick = (id) => {
    data.forEach((item) => {
      if (item._id === id) {
        ...
        let types = loanTypes.filter((data) => data.value === item.loan._id);
  ...
  const onActionClick = async(e) => {
    if (actionText === "delete") {
      ...
      obj['offer_banners'] = arr;
      
      try{
        const res = await fetch(BASE_URL + `offer-banner/bulk-delete`,{
            method : "post",
            // mode: 'no-cors',
            headers : {
                "Content-Type" : "application/json",
  ...
  return (
    <React.Fragment>
      <Head title="Offer Banner Lists"></Head>
      <Content>
        <BlockHead size="sm">
          <BlockBetween>
            <BlockHeadContent>
              ...
              <BlockDes className="text-soft">
                <p>You have total {data.length} offer banners.</p>
              </BlockDes>
            </BlockHeadContent>
            <BlockHeadContent>
          ...
          <DataTable className="card-stretch">
            ...
            <DataTableBody compact>
              ...
              <DataTableRow className="nk-tb-col-check">
                ...
                <DataTableRow>
                  <span className="sub-text">Loan</span>
                </DataTableRow>
                <DataTableRow size="md">
                  <span className="sub-text">Image</span>
                </DataTableRow>
                <DataTableRow size="md">
                  <span className="sub-text">Apply Path</span>
                </DataTableRow>
                ...
                ? currentItems.map((item) => {
                    return (
                      <DataTableItem key={item._id}>
                          ...
                          <div>
                            <div className="user-card">
                              <UserAvatar
                                theme={item.avatarBg}
                                className="xs"
                                text={findUpper(item.loan.name)}
                                image={''}
                              ></UserAvatar>
                              <div className="user-info">
                                <span className="tb-lead">{item.loan.name} </span>
                              </div>
                            </div>
                          </div>
                        </DataTableRow>
                        ...
                        <DataTableRow size="md">
                          <div style={{}}>
                              <img className="border border-info rounded" src={item.imageUrl}  alt={item.loan.name} style={{
                                  with : 120,
                                  height : 70,
                              }}/>
                          </div>
                ...
                <Form className="row gy-4" onSubmit={handleSubmit(onFormSubmit)} >
                  <Col md="6">
                    <FormGroup>
                      ...
                      <RSelect options={loanTypes} name="loanType" placeholder="Select Loan" defaultValue={formData.loanType} onChange={(e) => {
            ...
            <div className="p-2">
              ...
              <div className="mt-4">
                ...
                <Col md="6">
                    <FormGroup>
                    <label className="form-label">Loan</label>
                      <RSelect options={loanTypes} name="edit_loanType" placeholder="Select Loan" defaultValue={formData.edit_loanType} onChange={(e) => {
